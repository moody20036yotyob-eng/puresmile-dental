...
// ── Gzip compression (biggest speed win) ─────────────────
...
if (IS_PROD) {
  // Static assets: cache for 1 year (they have content hashes in filenames)
  app.use('/assets', express.static(join(CLIENT_DIR, 'assets'), {
    maxAge: '1y',
    immutable: true,
  }))
  // Everything else in dist: short cache
...
app.listen(PORT, () => {
  ...
  console.log(`📦  Mode: ${IS_PROD ? 'PRODUCTION (serving built client)' : 'DEV (API only)'}`)
